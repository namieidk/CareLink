...
class _DocSignInScreenState extends State<DocSignInScreen>
  ...
  // ──────────────────────────────────────────────────────────────
  // FORGOT PASSWORD (not implemented for doctors yet)
  // ──────────────────────────────────────────────────────────────
  Future<void> _handleForgotPassword() async {
    if (_identifierController.text.isEmpty) {
      _showError('Enter your Doctor ID or Username first');
      return;
    }
    _showSuccess('Password reset not available for doctors yet');
  }
  ...
  String? _validateIdentifier(String? v) =>
      (v == null || v.trim().isEmpty) ? 'Enter Doctor ID or Username' : null;
  ...
  Widget build(BuildContext context) {
    ...
    final double panelTop = size.height * 0.30;
